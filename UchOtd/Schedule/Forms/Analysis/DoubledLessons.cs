...
namespace UchOtd.Schedule.Forms.Analysis
{
    public partial class DoubledLessons : Form
    {
        ...
        {
            var notTheLastOnesAttrs = _repo.CustomDisciplineAttributes.GetFiltredCustomDisciplineAttributes(cda => cda.Key == "BetterTogether").ToList();
        ...
        private void Exclude_Click(object sender, EventArgs e)
        {
            ...
            foreach (var discId in discIds)
            {
                ...
                var notLastLessonAttribute = _repo
                    .CustomDisciplineAttributes
                    .GetFirstFiltredCustomDisciplineAttribute(cda => cda.Discipline.DisciplineId == localDiscId && cda.Key == "BetterTogether");

                if (notLastLessonAttribute != null)
                {
                    _repo.CustomDisciplineAttributes
                        .RemoveCustomDisciplineAttribute(notLastLessonAttribute.CustomDisciplineAttributeId);
                }
            }            
